...
let line_to_moves_with_cache_depth mp limit line =
  let chars = String.to_list line in
  let bad_cell = Map.find_exn mp '_' in

  let rec expand_char i prev current =
    if i >= limit then 1
    else
      let key = (i, prev, current) in
      let found = Stdlib.Hashtbl.find_opt cache key in

      match found with
        | Some v -> v
        | None ->
            let mvs = cells_to_mvs bad_cell prev current in
            let chars = String.to_list mvs in
            let _, ret =
              List.fold_left chars ~init:('A', 0)
                ~f:(fun (last_char, len) curr_char ->
                  let s = Map.find_exn mp last_char in
                  let e = Map.find_exn mp curr_char in
                  (curr_char, len + expand_char (i + 1) s e))
            in
            let () = Stdlib.Hashtbl.add cache key ret in
            ret
  in

  let _, out =
    List.fold_left chars ~init:('A', 0) ~f:(fun (last_char, len) curr_char ->
        let s = Map.find_exn mp last_char in
        let e = Map.find_exn mp curr_char in
        (curr_char, len + expand_char 0 s e))
  in
  out
...
let () =
  ...
  let aggr_len a s =
    let len =
      s |> line_to_moves numpad |> line_to_moves_with_cache_depth mvpad 25
    in

    a + (len * get_number_prefix s)
  in

  List.fold_left lines ~init:0 ~f:aggr_len |> printf "%d\n";
  ()
