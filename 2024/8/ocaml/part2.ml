...
let getHotspots isInBounds pair =
  ...
  let getSpots dir =
    let rec loopFn i spots =
      let newSiteI = a1 0 + (dir * i * diffi) in
      let newSiteJ = a1 1 + (dir * i * diffj) in
      if not (isInBounds newSiteI newSiteJ) then spots
      else
        let newSpots = [ newSiteI; newSiteJ ] :: spots in
        loopFn (i + 1) newSpots
    in
    loopFn 0 []
  in
  ([ a1 0; a1 1 ] :: getSpots 1) @ getSpots (-1)
