  ...
  let rec traverse seen options =
    match Map.is_empty options with
      ...
      | false ->
          ...
          if Poly.equal current_cell finish then Some current_cost
          else
...
(* --- searching backwards is much faster -- *)
(* --- can make a bin search if need more speed - lazy -- *)
let rec loop blocks last =
  let filtered_blocks = blocks |> Set.Poly.of_list in
  let cost = traverse filtered_blocks in
  match cost with
    | Some _ -> last
    | None ->
    match blocks with
      | h :: tail -> loop tail h
      | [] -> failwith "ran out of blocks in loop"
...
let () =
  ...
  let blocks = get_nums s |> List.map ~f:Int.of_string |> to_pairs in

  let fx, fy = loop blocks (0, 0) in
  printf "%d,%d\n" fx fy;
  ()
